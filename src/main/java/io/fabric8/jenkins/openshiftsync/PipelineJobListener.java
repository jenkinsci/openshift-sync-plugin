...
public class PipelineJobListener extends ItemListener {
    ...
    public void onDeleted(Item item) {
        ...
        if (item instanceof WorkflowJob) {
            ...
            if (job.getProperty(BuildConfigProjectProperty.class) != null
                    && isNotBlank(job.getProperty(
                            BuildConfigProjectProperty.class).getNamespace())
                    && isNotBlank(job.getProperty(
                            BuildConfigProjectProperty.class).getName())) {
    ...
    public void upsertItem(Item item) {
        if (item instanceof WorkflowJob) {
            WorkflowJob job = (WorkflowJob) item;
            if (job.getProperty(BuildConfigProjectProperty.class) != null
                    && StringUtils.isNotBlank(job.getProperty(
                            BuildConfigProjectProperty.class).getNamespace())
                    && StringUtils.isNotBlank(job.getProperty(
                            BuildConfigProjectProperty.class).getName())) {
                logger.info("Updated WorkflowJob " + job.getDisplayName()
                        + " replicating changes to OpenShift");
    ...
    private void upsertBuildConfigForJob(WorkflowJob job) {
        ...
        if (buildConfigProjectProperty == null
                || buildConfigProjectProperty.getNamespace() == null
                || buildConfigProjectProperty.getName() == null
                || buildConfigProjectProperty.getUid() == null) {
            return;
        }

        BuildConfig jobBuildConfig = buildConfigProjectProperty
                .getBuildConfig();
        if (jobBuildConfig == null) {
            logger.log(
                    Level.WARNING,
                    "Failed to find BuildConfig in namespace: "
                            + buildConfigProjectProperty.getNamespace()
                            + " for name: "
        ...
        if (bcUpdated) {
            try {
                getAuthenticatedOpenShiftClient()
                        .buildConfigs()
                        .inNamespace(
                                jobBuildConfig.getMetadata().getNamespace())
                        .withName(jobBuildConfig.getMetadata().getName())
                        .cascading(false).replace(jobBuildConfig);
            } catch (Exception e) {
                logger.log(Level.WARNING, "Failed to update BuildConfig: "
